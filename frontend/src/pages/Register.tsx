...
const Register: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response: AxiosResponse = await axios.post('http://localhost:8080/api/users/register', {
        username,
        email,
        password,
      });

      localStorage.setItem('token', response.data.token); // Aqui você pode adicionar o token ao localStorage
      navigate('/login');
    } catch (err) {
